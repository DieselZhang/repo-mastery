...
3. **Functional differentiation vs peers（功能差异点）** — 2–4 lines, tagged
   ...
   「需验证」, never fabricated; **preview does not persist `positioning.md`** —
...
<repo>` directly: the preview brief (architecture narrative + differentiation +
highlights) feeds the Phase 2 value brief, skipping the redundant scouting and
going straight to the Phase 1 course-map proposal + Phase 2 Mission/confirmation.
preview persists nothing; `start` builds the formal files from the brief
(MISSION.md / positioning.md / course-map.json).
